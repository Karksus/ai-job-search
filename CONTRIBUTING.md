...
**This repo is a universal template.** People fork it and adapt it to their own market, language, and profile. Upstream stays market-agnostic, person-agnostic, and AI-tool-agnostic. The corollary: a contribution is judged by fit to this rule first, execution quality second. Well-built but off-policy still gets declined (kindly, with reasons).

## What gets merged

- **Universal customization features**: anything that makes the fork-and-adapt path better for everyone.
...
- **Alternative-harness ports and duplicate workflow sources.** The markdown specs ARE the implementation; a second copy (another orchestration layer, a wrapper command) drifts from the first the moment either changes. The Python orchestrator (`tools/orchestrate.py`) is the exception — it reads the markdown specs at runtime and doesn't duplicate them.
...
The core lifecycle is **feature-complete**: profile setup → job search → job evaluation → application → interview prep → outcome tracking, with skill gap analysis, profile enrichment, and template management around it. Every stage of a real job hunt has an owner.
...
1. Fork the repo and create a new portal skill in `.agents/skills/<name>/` following the same structure as the shipped ones (TypeScript CLI, `search`/`detail` commands, `--format json|table|plain`, stderr JSON errors).
...
- **Portal-skill contract**: `search`/`detail` commands, `--format json|table|plain`, stderr JSON errors with exit 1, backoff on 429/5xx, zero runtime dependencies by default. See `linkedin-search` as the reference implementation.
